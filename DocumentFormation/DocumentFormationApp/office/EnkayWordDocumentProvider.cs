...
{
    public class EnkayWordDocumentProvider:IEnkayDocumentProvider
    {
        private WordService service;


        public EnkayWordDocumentProvider(WordService service)
        {
            this.service = service;
        }


        public EnkayWordDocumentProvider(string orderDocumentFileName)
        {
        ...
        {
            var lotList = service.GetCell(1, 8, 2);
            ...
            pattern = @"\b(Лот №\d+|Лот №\W?\d+)\b(.[\D\s]*)\W(([\d\s]*[,\.]{1}\d{2})|([\d\s]*))\s\(.";
        ...
        private void DeleteUnnecessaries()
        {
            try {
                ...
                service.FindReplace("конкурсных ", "");
                service.FindReplace("конкурсной ", "");
                service.DeleteTableRows(1, new int[10] {11, 1, 1, 1, 1, 1, 1, 1, 1, 1 });
            } catch { }
        }
